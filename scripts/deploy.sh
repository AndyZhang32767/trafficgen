...
#   客户端:  sudo ./deploy.sh client http://SERVER_IP:8080/stream 4
set -e
...
case "$ROLE" in
  server)
    ...
    echo "服务端已启动。面板: http://<本机IP>:8080/  ，刷流量入口: /stream"
    systemctl status trafficgen-server --no-pager -l | head -n 5
    ;;
  client)
    URL="${2:-}"
    CONN="${3:-4}"
    if [ -z "$URL" ]; then echo "用法: sudo ./deploy.sh client http://SERVER_IP:8080/stream [并发]"; exit 1; fi
    build client
    sed "s#http://SERVER_IP:8080/stream#$URL#; s#-c 4#-c $CONN#" \

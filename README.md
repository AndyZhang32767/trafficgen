...
最省事的用法：服务端启动后，**用浏览器打开 `http://服务端IP:8080/` 即可**——页面本身就是客户端，打开就自动连接开始刷流量（可在页面上设并发数、随时停止）。若要更高吞吐，再用下面的 Go / curl 客户端。
...
- 访问 `http://服务端IP:8080/stream` 即开始刷流量：服务端持续 `Write` 数据，客户端持续读取并 `丢弃(io.Discard / /dev/null)`。
...
完成后浏览器打开 `http://你的服务端IP:8080/` 查看面板。
（记得在云厂商安全组/防火墙放行 8080 端口。）

手动运行（不装服务）也可以：

```bash
go build -o server ./server
./server -addr :8080 -stats stats.json
```
...
浏览器直接访问 `http://服务端IP:8080/`，页面加载后会自动作为客户端连接并开始刷流量。可在页面上调整“并发连接”并点“停止/开始”。
注意：HTTP/1.1 下浏览器对同一站点最多约 6 个并发连接，所以网页客户端单页吞吐有上限；要更快请用下面的 Go 客户端或多开几个浏览器标签/机器。

**方式 B：Go 客户端（吞吐高）**

```bash
sudo scripts/deploy.sh client http://服务端IP:8080/stream 4
```
...
scripts/client.sh http://服务端IP:8080/stream 4
```
...
客户端 `client`：
- `-url`：服务端流地址，如 `http://IP:8080/stream`
- `-c`：并发连接数，默认 `4`
